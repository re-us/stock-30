    ...
    <section className="space-y-3">
      <div className="rounded-[20px] bg-[#191f28] p-3 text-white shadow-[0_8px_20px_rgba(25,31,40,0.10)] sm:p-3.5">
        ...
        <div className="mt-2 flex items-start justify-between gap-4">
          <div className="min-w-0">
            <h2 className="truncate text-[21px] font-black leading-6 sm:text-2xl">{getStockDisplayName(top)}</h2>
        ...
        <div className="mt-2 grid grid-cols-2 gap-1.5">
          <MiniMetric label="전일대비 관심량 증가" value={formatPercent(top.mentionChangeRate)} />
          <MiniMetric label="이번주 상승확률 (데이터 기반 예측)" value={top.weeklyUpsideProbability ? `${top.weeklyUpsideProbability.probability}%` : "-"} />
        </div>
      </div>
      <div className="grid grid-cols-2 gap-2.5 sm:grid-cols-3">
        <div className="rounded-[20px] bg-white p-3.5 shadow-[0_6px_18px_rgba(15,23,42,0.035)]">
          ...
          <p className="mt-1.5 truncate text-base font-black text-slate-950">{getStockDisplayName(fastest)}</p>
        ...
        <div className="rounded-[20px] bg-white p-3.5 shadow-[0_6px_18px_rgba(15,23,42,0.035)]">
          ...
          <p className="mt-0.5 text-[11px] font-bold text-slate-400">전체 종목 전일대비 관심량 증가</p>
...
function MiniMetric({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-[14px] bg-white/10 px-2.5 py-1.5">
      <p className="break-keep text-[11px] font-bold leading-4 text-slate-300">{label}</p>
