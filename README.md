...
이 서비스는 투자 추천 서비스가 아니며, 지표는 참고 분석 자료입니다.
...
민감한 키에는 `NEXT_PUBLIC_` 접두사를 사용하지 않습니다. 외부 데이터 호출은 서버 Route Handler에서만 실행됩니다.
...
`vercel.json`에 6시간 주기 cron 설정이 포함되어 있습니다.
...
Vercel Hobby 플랜에서는 Cron 실행 횟수 제한이 있을 수 있습니다. 6시간마다 자동 업데이트가 필요하면 Vercel Pro 플랜 또는 외부 cron 서비스를 고려해야 합니다.

현재 저장소나 DB는 연결하지 않았습니다. 서버리스 환경의 in-memory cache는 영구 저장소가 아니므로, 추후 Supabase, Vercel KV, Redis 등을 연결할 수 있도록 캐시 함수가 분리되어 있습니다.

## 보안 및 운영 주의사항

- API 키는 Vercel Environment Variables에만 입력합니다.
- 민감한 값에는 `NEXT_PUBLIC_` 접두사를 사용하지 않습니다.
- 외부 API 호출은 서버 Route Handler와 서버 유틸에서만 실행합니다.
- 무료 API는 호출량 제한이 있을 수 있으므로 Cron 주기와 수동 새로고침 빈도를 확인합니다.
- Vercel Hobby 플랜은 Cron 실행 횟수 제한이 있을 수 있습니다.
- 현재 in-memory cache는 영구 저장소가 아닙니다.
- 실제 운영에서는 Supabase, Vercel KV, Redis 같은 저장소와 rate limit 저장소를 연결하는 것을 권장합니다.
- 이 서비스는 투자 추천 서비스가 아니며 참고 정보 서비스입니다.
- CORS 허용 헤더를 별도로 추가하지 않았습니다. 필요 시 allowlist 방식으로 확장합니다.
